...
//           }
        ...
        if((selectedMonths.compareTo(remainingMonths))>0)
        {
                ...
                JSFUtils.addFacesErrorMessage("Number of months shouldn't exceed "+remainingMonths+" month/s");
                return null;
        ...
        OperationBinding oper =
            ADFUtils.findOperation("getMobileRequestsThisMonth");
        Map paramMap = oper.getParamsMap();
        paramMap.put("PersonNumber",
                     ADFUtils.getBoundAttributeValue("PersoneNumber"));
        oracle.jbo.domain.Number totaRequestsThisMonth =
            (oracle.jbo.domain.Number)oper.execute();
...
//            }
        ...
        try {
            reportvalues =
                    bi.getmobileDatFileData(ADFUtils.getBoundAttributeValue("PersoneNumber").toString());   
        ...
        String personNumber =
            ADFUtils.getBoundAttributeValue("PersoneNumber").toString();
...
//        System.err.println("RequestId=======>"+ADFUtils.getBoundAttributeValue("RequestId")+"stepid====="+stepid+"nexstep===>"+nextStep.getAttribute("NextStepId")+"AssigneeName===>"+ADFUtils.getBoundAttributeValue("AssigneeName"));
       String value= ApprovalHistory.executeHistoryPackage((oracle.jbo.domain.DBSequence)ADFUtils.getBoundAttributeValue("RequestId"), 
                                                                    "MobileAllowance", (Number)nextStep.getAttribute("NextStepId"), 
                                                                    stepid, 
                                                                    (String)ADFUtils.getBoundAttributeValue("AssigneeName"), 
                                                                    act, "");
       System.err.println("values======"+value);
        Row mobileAllowanceRow =
            ADFUtils.findIterator("MobileAllowanceRequestUPViewIterator").getCurrentRow();
...
//        ADFUtils.findOperation("Execute").execute();
    ...
    public void sendEmailByEmail(String personEmail, Row subject) {
        ...
        sendMobileAllowanceEmail("OFOQ.HR@TATWEER.SA", personEmail,
                                 (MobileAllowanceRequestUPViewRowImpl)subject , "");
        JSFUtils.addFacesInformationMessage("Mail has been sent");

    }

    public void sendMobileAllowanceEmail(String from, String to,
                                         MobileAllowanceRequestUPViewRowImpl subject, String personFYI) {
...
//            "Kindly find below the details of Mobile Allowance request as below" +
        ...
        String signaturePart =
            ...
            "<b/>" + "<br/>";
        String emailcontent =
            into + personalInformation + verticalSpace + MobileAllowanceDetails +verticalSpace+
    ...
    public void sendMobileAllowanceEmailToPayrollMgr(String from, String to,
                                                     MobileAllowanceRequestUPViewRowImpl subject,
                                                     String payrollMgrName) {
        if (to == null) {
           // to = "heleraki@gmail.com";
           JSFUtils.addFacesErrorMessage("Email Address is not updated! please contact HR Administrator!");
        ...
        String signaturePart =
            ...
            "<b/>" + "<br/>";
        String emailcontent =
                    into + personalInformation + verticalSpace + MobileAllowanceDetails +
                    ApprovalPart1+thankYouPart + signaturePart + "</p>";
        ...
        sendMail.getParamsMap().put("subject",
                                    "Mobile Allowance Request " + subject.getRequestStatus());
        sendMail.getParamsMap().put("e_body", emailcontent);
        sendMail.execute();
    }
...
//        }
        ...
        if (nextStep.getAttribute("NextAssignee").equals("Finished")) {
            UserServiceUtil userService = new UserServiceUtil();
             personNumber =
                JSFUtils.resolveExpression("#{bindings.PersoneNumber.inputValue}").toString();
            UserDetails byPersonNumber =
                userService.getUserDetailsByPersonNumber(personNumber);
            ADFUtils.setBoundAttributeValue("Assignee",
                                            byPersonNumber.getPersonNumber());
            ADFUtils.setBoundAttributeValue("AssigneeName",
                                            "");
            ...
            String MultipleEntryCount =
                datFileData.get(0).get("MULIPLE_ENTRY")!=null? datFileData.get(0).get("MULIPLE_ENTRY").toString():"";   
...
//        sendEmailForMobileAllowanceEmployee(fyiEmailAddress, mobileAllowanceRow);
//        sendEmailForMobileAllowanceEmployee("1062", mobileAllowanceRow);
//        sendEmailToPayrollMgrFinalApproval(mobileAllowanceRow);
        }
        }
    ...
    public void sendEmail(String personNumber, Row subject) {
        ...
        if (null == email) {
            ...
            sendMobileAllowanceEmail("OFOQ.HR@TATWEER.SA", email,
                                     (MobileAllowanceRequestUPViewRowImpl)subject, "");
            JSFUtils.addFacesInformationMessage("Mail has been sent");
        }
    }
    ...
    public void sendEmailToPayrollMgrFinalApproval(Row subject) {
        ...
        if (null == (personData.get(0).get("EMAIL_ADDRESS"))) {
            ...
            sendMobileAllowanceEmailToPayrollMgr("OFOQ.HR@TATWEER.SA",
                                                 personData.get(0).get("EMAIL_ADDRESS").toString(),
                                                 (MobileAllowanceRequestUPViewRowImpl)subject,
...
//            String email = getEmail(personNumber);
            ...
            if (null == email) {
                ...
                sendMobileAllowanceEmailForEmployee("OFOQ.HR@TATWEER.SA", email,
                                         (MobileAllowanceRequestUPViewRowImpl)subject);
                JSFUtils.addFacesInformationMessage("Mail has been sent");
            }
        }
    public void sendMobileAllowanceEmailForEmployee(String from, String to,
                                         MobileAllowanceRequestUPViewRowImpl subject) {
        if (to == null) {
          //  to = "heleraki@gmail.com";
          JSFUtils.addFacesErrorMessage("Email Address is not updated! please contact HR Administrator!");
    ...
    public String validateMobileAllowance(String start_date,String end_date) {
        String returnvalue = "";
        ViewObject vo =
            ADFUtils.findIterator("MobileAllowanceRequestUPViewIterator").getViewObject();
        oracle.jbo.domain.Date date =
            (oracle.jbo.domain.Date)vo.getCurrentRow().getAttribute("CreationDate");
        try {
            oracle.jbo.domain.Date tooracledate1 =
                new oracle.jbo.domain.Date(start_date);
            oracle.jbo.domain.Date tooracledate2 =
                new oracle.jbo.domain.Date(end_date);  
            System.err.println(tooracledate1+""+tooracledate2);
                if(tooracledate1.compareTo(date) *
                                   date.compareTo(tooracledate2)>0) {
                
                    returnvalue=  "failed";
                }
            else {
                    long value=tooracledate1.compareTo(date) *
                                       date.compareTo(tooracledate2);
                    returnvalue=  "success";
                }
    ...
    public String changeEffectiveEndDate() {
        ...
        ViewObject vo =
            ADFUtils.findIterator("MobileAllowanceRequestUPViewIterator").getViewObject();
        Number countofentry =
            (Number)ADFUtils.getBoundAttributeValue("NumberOfMonths");
        oracle.jbo.domain.Date date =
            (oracle.jbo.domain.Date)vo.getCurrentRow().getAttribute("CreationDate");   
    ...
    public String reject_withdraw() {
        ...
        ADFUtils.findOperation("Commit").execute();
        String value =
            ApprovalHistory.executeHistoryPackageWithdraw((oracle.jbo.domain.DBSequence)ADFUtils.getBoundAttributeValue("RequestId"),
                                                      "MobileAllowance",
                                                 (Number)nextStep.getAttribute("StepId"),
                                                  new Long(0),
                                                  null,
    ...
    public void sendEmailToPerson(String personNumber, Row subject) {
        ...
        if (null == email) {
            ...
            sendMobileAllowanceEmail("OFOQ.HR@TATWEER.SA", email,
                                     (MobileAllowanceRequestUPViewRowImpl)subject, "Y");
            JSFUtils.addFacesInformationMessage("Mail has been sent");
        }
    } 
